...
from .serializers import ParcelsSerializer, ParcelsOwnersSerializer
...
class HelloWord(View):
    def get(self, request):
        return JsonResponse({"ok":True,"message": "Hello world from app. Parcels", "data":[]})        

class ParcelsView(BaseDjangoView):
    ...
    #     GET /parcels_view/selectone/<id>/
    # To get all the records, the URL must be like:
    #     GET /parcels_view/selectall/
    # To insert a record, the URL must be like:
    #     POST /parcels_view/insert/ --> The data must be sent in the body of the request.
    # To update a record, the URL must be like:
    #     POST /parcels_view/update/<id>/ --> The data must be sent in the body of the request.
    # To delete a record, the URL must be like:
    #     POST /parcels_view/delete/<id>/
    ...
    # tu pa to splošno metodo prilagodimo tako, da deluje samo za model Parcels.
    # link: http://localhost:8000/parcels/parcels_view/selectone/1/
    def selectone(self, id):                        # metoda selectone je definirana v razredu BaseDjangoView 
        ...
        return JsonResponse({'ok':True, 'message': 'Parcel Retriewed', 'data': [d]}, status=200)

    # podobna metoda kot zgoraj, samo da vrne vse objekte Parcels
    # Link: http://localhost:8000/parcels/parcels_view/selectall/
    def selectall(self):
        l=Parcels.objects.all()[:MAX_NUMBER_OF_RETRIEVED_ROWS]
        data=[]
        for b in l:
            d=model_to_dict(b)
            d['geom']=b.geom.wkt
            data.append(d)
        return JsonResponse({'ok':True, 'message': 'Parcel data retrieved', 'data': data}, status=200)

    #POST OPERATIONS
    # Tudi metoda insert ni samostojna URL metoda, ampak je metoda, ki jo lahko pokličeš iz razreda BaseDjangoView.
    # link: POST http://localhost:8000/parcels/parcels_view/insert/        INSERT torej POST torej POSTMAN
    ...
    def insert(self, request):
        ...
        # After the parcel has been inserted:
        #     - snap it to grid
        ...
        b=Parcels(description=description, area=g.area, geom=g)
        b.save()
        print(f"Geometry inserted id: {b.id}")

        #Update the geometry to an snaped one to the grid
        Parcels.objects.filter(id=b.id).update(geom=SnapToGrid('geom', ST_SNAP_PRECISION))
        ...
        filt=Parcels.objects.filter(geom__relate=(g.wkt,'T********')).exclude(id=b.id)   # te filtre že poznamo iz prejšnjih vaj
    ...
    def update(self, request, id):
        ...
        if originalWkt is not None:                        # če imamo geometrijo, jo pretvorimo v WKB format
            ...
            interesectionIds=gc.check_st_relate('parcels_parcels','T********', id_to_avoid=id) # Preverimo, ali se geometrija sekajo z drugimi geometrijami
            ...
            b.area=polyGeos.area                  # izračunamo površino nove geometrije
            b.save()
            d=model_to_dict(b)
    ...
    # link: POST http://localhost:8000/parcels/parcels_view/insert2/
    def insert2(self, request):
        ...
        if originalWkt is not None:
            ...
            isValid=gc.is_geometry_valid()               # Preverimo, ali je geometrija veljavna
            gc.check_st_relate('parcels_parcels','T********')    # Preverimo, ali se geometrija sekajo z drugimi geometrijami
            ...
            p.parc_st=request.POST.get('parc_st', '')   # iz POST zahteve izluščimo parcelno številko
            p.sifko=request.POST.get('sifko', '')   # iz POST zahteve izluščimo še šifro KO
            p.area=p.geom.area                                  # izračunamo površino nove geometrije
            p.save()                                            # shranimo objekt Parcels v bazo
            d=model_to_dict(p)
            d['geom']=p.geom.wkt
...
class ParcelsModelViewSet(viewsets.ModelViewSet):
    #     GET operation over /parcels/parcels/. It will return all reccords
    #     GET operation over /parcels/parcels/<id>/. 
    #     POST operation over /parcels/parcels/. It will insert a new record
    #     PUT operation over /parcels/parcels/<id>/. 
    #     PATCH operation over /parcels/parcels/<id>/. 
    #     DELETE operation over /parcels/parcels/<id>/. 
    queryset = Parcels.objects.all().order_by('id')  # izpis na front-endu bo urejen po id-ju
    serializer_class = ParcelsSerializer           
    permission_classes = [permissions.AllowAny]

                                

class ParcelOwnersModelViewSet(viewsets.ModelViewSet):
    #     -list()  ->           GET operation over /parcels/owners/. It will return all reccords
    #     -retrieve() ->        GET operation over /parcels/owners/<id>/. 
    #     -create() ->          POST operation over /parcels/owners/. It will insert a new record
    #     -update() ->          PUT operation over /parcels/owners/<id>/. 
    #     -partial_update() ->  PATCH operation over /parcels/owners/<id>/. 
    #     -destroy() ->         DELETE operation over /bparcels/owners/<id>/. 
    queryset = Parcels_Owners.objects.all()
    serializer_class = ParcelsOwnersSerializer            
    permission_classes = [permissions.AllowAny]
